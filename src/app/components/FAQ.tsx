    ...
    <div className="py-20 px-6 max-w-7xl mx-auto flex flex-col md:flex-row gap-12">
      ...
      >
        ...
        <a
          ...
          className="inline-block w-48 bg-teal-500/10 border border-teal-500/30 text-teal-600 dark:text-teal-400 hover:bg-teal-500/20 px-6 py-3 rounded-full font-bold shadow-[0_0_20px_rgba(20,184,166,0.15)] transition-all"
        >
          Contact Support
        </a>
      </motion.div>
      ...
      <div className="md:w-2/3 space-y-4">
        {faqs.map((faq, i) => (
          <motion.div
            ...
            className="border border-border rounded-xl bg-card overflow-hidden hover:border-teal-500/30 transition-colors duration-300"
          >
            <button
              className="w-full flex items-center justify-between p-6 text-left"
              onClick={() => setOpenIndex(openIndex === i ? null : i)}
            >
              <span className="font-medium text-foreground transition-colors duration-800">{faq.q}</span>
              <motion.div
                animate={{ rotate: openIndex === i ? 180 : 0 }}
                transition={spring.snappy}
              >
                <ChevronDown
                  className={`w-5 h-5 transition-colors duration-300 ${openIndex === i ? "text-teal-500 dark:text-teal-400" : "text-muted-foreground/40"}`}
                />
              </motion.div>
            </button>
            <AnimatePresence>
              {openIndex === i && (
                ...
                >
                  <div className="px-6 pb-6 text-muted-foreground text-sm leading-relaxed border-t border-border pt-4 transition-colors duration-800">
                    {faq.a}
                  </div>
                </motion.div>
              )}
